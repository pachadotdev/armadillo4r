...
doubles_matrix<> mat_demo_(const doubles_matrix<>& x,
  ...
  // use A, Ai, Au, Af ...
  mat result = A * A.t();         // materialize before converting
...
doubles_matrix<> mat_out_demo_(const doubles_matrix<>& x) {
  mat A = as_Mat(x);
  mat B = A.t() * A;            // A'A
  return as_doubles_matrix(B);  // mat -> doubles_matrix<>
}
```
...
doubles vec_out_demo_(const doubles& y) {
  vec v    = as_Col(y);
  vec vnorm = normalise(v);     // L2-normalise
  return as_doubles(vnorm);     // vec -> doubles
}
```
...
SEXP as_sexp_demo_(const doubles& y) {
  vec v = as_Col(y);
  vec w = sort(v);
  return cpp4r::as_sexp(w);  // vec -> doubles (dispatched automatically)
}
```
