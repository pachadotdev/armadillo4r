...
`armadillo4r`, as of v0.5.0, supports the `dgCMatrix` class from the
...
Note that `cpp4r` does not provide sparse matrices as it is the case for
the dense data types `doubles_matrix<>` or `integers_matrix<>`. `armadillo4r`
