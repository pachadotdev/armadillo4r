#!/usr/bin/env bash
# Docker-based R CMD check for cpp4r / armadillo4rtest.
#
...
ARMADILLO4RTEST_TARBALL_FILE=$(mktemp)
trap 'rm -rf "$CHECK_DIR" "$CPP4R_TARBALL_FILE" "$ARMADILLO4R_TARBALL_FILE" "$ARMADILLO4RTEST_TARBALL_FILE"' EXIT

Rscript -e 'tinydev::pkg_register("./armadillo4rtest")' >>"$BUILD_LOG" 2>&1
Rscript -e 'tinydev::pkg_document("./armadillo4rtest")' >>"$BUILD_LOG" 2>&1

Rscript -e "writeLines(tinydev::pkg_build('../cpp4r'), '${CPP4R_TARBALL_FILE}')" >>"$BUILD_LOG" 2>&1

Rscript -e "writeLines(tinydev::pkg_build('.'), '${ARMADILLO4R_TARBALL_FILE}')" >>"$BUILD_LOG" 2>&1
Rscript -e "writeLines(tinydev::pkg_build('./armadillo4rtest'), '${ARMADILLO4RTEST_TARBALL_FILE}')" >>"$BUILD_LOG" 2>&1

CPP4R_TARBALL=$(cat "$CPP4R_TARBALL_FILE")
ARMADILLO4R_TARBALL=$(cat "$ARMADILLO4R_TARBALL_FILE")
ARMADILLO4RTEST_TARBALL=$(cat "$ARMADILLO4RTEST_TARBALL_FILE")

CPP4R_FILE=$(basename "$CPP4R_TARBALL")
ARMADILLO4R_FILE=$(basename "$ARMADILLO4R_TARBALL")
ARMADILLO4RTEST_FILE=$(basename "$ARMADILLO4RTEST_TARBALL")

cp "$CPP4R_TARBALL" "$CHECK_DIR/"
cp "$ARMADILLO4R_TARBALL" "$CHECK_DIR/"
cp "$ARMADILLO4RTEST_TARBALL" "$CHECK_DIR/"
...
R_EOF
sed -i \
  -e "s|__TARBALLS__|'/check/${CPP4R_FILE}', '/check/${ARMADILLO4RTEST_FILE}'|" \
  -e "s|__OWN_NAMES__|'cpp4r', 'armadillo4rtest'|" \
...
# armadillo4rtest/configure regenerates src/Makevars from Makevars.in, honoring
# this env var for CXX_STD (defaulting to CXX23 when unset/empty).
ARMADILLO4RTEST_CXX_STD=""
if [ -n "$STD" ]; then
  ARMADILLO4RTEST_CXX_STD=$(echo "$STD" | tr '[:lower:]' '[:upper:]')
fi
...
docker run --rm \
  ...
  bash -c "
    ...
    show_logs_and_fix_perms() {
      echo '=== 00install.out ==='
      cat /check/armadillo4rtest.Rcheck/00install.out || true
      echo '=== 00check.log ==='
      cat /check/armadillo4rtest.Rcheck/00check.log || true
      chmod -R a+rwX /check
    }
...
${MAKEVARS_STEP}
    # Remove stale locks and old cpp4r/armadillo4rtest before reinstalling
    rm -rf /cache/R_libs/00LOCK-* /cache/R_libs/cpp4r /cache/R_libs/armadillo4rtest
    export ARMADILLO4RTEST_CXX_STD='${ARMADILLO4RTEST_CXX_STD}'
    R CMD INSTALL --library=/cache/R_libs /check/${CPP4R_FILE}
    R CMD INSTALL --library=/cache/R_libs /check/${ARMADILLO4R_FILE}
    R CMD INSTALL --library=/cache/R_libs /check/${ARMADILLO4RTEST_FILE}
    cd /check
    export _R_CHECK_FORCE_SUGGESTS_=false
    R CMD check --as-cran --no-manual ${ARMADILLO4RTEST_FILE}
  " 2>&1 | grep -v 'readelf: Warning:' | tee "${CHECK_DIR}/docker.log" || DOCKER_RC="${PIPESTATUS[0]}"

if awk 'found { print; next } /^\*\* this is package .*armadillo.* version/ { found=1; print }' "${CHECK_DIR}/docker.log" > "${CHECK_DIR}/docker.trimmed.log" && [ -s "${CHECK_DIR}/docker.trimmed.log" ]; then
  cp "${CHECK_DIR}/docker.trimmed.log" "$LOG"
else
  cp "${CHECK_DIR}/docker.log" "$LOG"
fi

if [ -d "${CHECK_DIR}/armadillo4rtest.Rcheck" ]; then
  RCHECK_DEST="${LOG_DIR}/${SUFFIX}-armadillo4rtest.Rcheck"
  rm -rf "$RCHECK_DEST"
  cp -r "${CHECK_DIR}/armadillo4rtest.Rcheck" "$RCHECK_DEST"
  rm "$BUILD_LOG"
