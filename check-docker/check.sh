...
mkdir -p "$LOG_DIR"
mkdir -p "$CACHE_DIR"
trap 'rm -rf "$CHECK_DIR"; rm -f "${CPP4R_TARBALL:-}" "${ARMADILLO4R_TARBALL:-}" "${ARMADILLO4RTEST_TARBALL:-}"' EXIT
...
ARMADILLO4R_TARBALL=$(Rscript -e 'cat(devtools::build(".", quiet = TRUE))')
ARMADILLO4RTEST_TARBALL=$(Rscript -e 'cat(devtools::build("./armadillo4rtest", quiet = TRUE))')
...
docker run --rm \
  ...
  bash -c "
    ...
    # Remove stale locks and old armadillo4r/armadillo4rtest before reinstalling
    rm -rf /cache/R_libs/00LOCK-* /cache/R_libs/armadillo4r /cache/R_libs/armadillo4rtest
...
if [ -d "${CHECK_DIR}/armadillo4rtest.Rcheck" ]; then
  RCHECK_DEST="${LOG_DIR}/${IMAGE}-armadillo4rtest.Rcheck"
  rm -rf "$RCHECK_DEST"
