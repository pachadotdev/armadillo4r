...
# mat_demo_: matrix conversions produce correct output
local({
...
# mat_demo_: accepts integer matrix for xi
local({
...
# vec_demo_: element-wise square is correct
local({
  y  <- c(1.0, 2.0, 3.0)
  yi <- 1L:3L
...
# vec_demo_: works for a single element
local({
...
# cube_col_means_: per-slice column means are correct
local({
  slices <- list(
...
# cube_col_means_: throws on mismatched matrix dimensions
local({
  bad_slices <- list(
    matrix(1:6, nrow = 2),
...
# cube_col_means_: throws on empty list
local({
...
# field_mat_demo_: L2-normalises each column of each matrix
local({
  mats <- list(
...
# field_mat_demo_: handles ragged matrices (different nrow)
local({
  mats <- list(
...
# field_imat_demo_: integer field roundtrip is lossless
local({
  mats <- list(
    matrix(1L:6L,  nrow = 2),
...
# field_imat_demo_: handles single-element field
local({
...
# field_vec_demo_: doubles each element of each vector
local({
...
# field_vec_demo_: handles ragged vectors (different lengths)
local({
...
# field_str_demo_: appends '!' to every string
local({
...
# field_str_demo_: works on a single string
local({
...
# mat_out_demo_: computes A'A correctly
local({
...
# vec_out_demo_: L2-normalises a vector
local({
  y <- c(3.0, 4.0)
...
# vec_out_demo_: result has unit L2 norm
local({
  set.seed(1)
  y      <- rnorm(10)
  result <- vec_out_demo_(y)
...
# random_cube_: returns a list of matrices with correct dims
local({
  set.seed(42)
...
# random_cube_: all elements are numeric matrices
local({
  set.seed(1)
...
# integer_cube_roundtrip_: integer slices are preserved
local({
  slices <- list(
    matrix(1L:4L,  nrow = 2),
    matrix(5L:8L,  nrow = 2),
...
# build_field_: returns list with correct lengths
local({
  set.seed(42)
  sizes  <- c(3L, 5L, 2L)
...
# build_field_: each element is a numeric vector
local({
  set.seed(1)
...
# as_sexp_demo_: sorts a vector in ascending order
local({
...
# as_sexp_demo_: already-sorted input is unchanged
local({
